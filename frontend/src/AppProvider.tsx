...
type Action =
    ...
    | { type: "UPDATE_TASK"; payload: { taskListId: string; taskId: string; task: Task } }
...
const reducer = (state: AppState, action: Action): AppState => {
  switch (action.type) {
    ...
    case DELETE_TASKLIST:
      return { ...state, taskLists: state.taskLists.filter((wl) => wl.id !== action.payload) };
    case FETCH_TASKS:
    ...
    case GET_TASK: {
      ...
      return { ...state, tasks: { ...state.tasks, [action.payload.taskListId]: updatedTasks } };
    }
    case UPDATE_TASK:
      return {
        ...state,
...
const initialState: AppState = { taskLists: [], tasks: {} };
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const api: AppContextType["api"] = {
    fetchTaskLists: async () => {
      const response = await axios.get<TaskList[]>(`${BASE_URL}/api/task-lists`, jsonHeaders);
      dispatch({ type: FETCH_TASKLISTS, payload: response.data });
    },
    getTaskList: async (id: string) => {
      const response = await axios.get<TaskList>(`${BASE_URL}/api/task-lists/${id}`, jsonHeaders);
      dispatch({ type: GET_TASKLIST, payload: response.data });
    },
    createTaskList: async (taskList) => {
      const response = await axios.post<TaskList>(`${BASE_URL}/api/task-lists`, taskList, jsonHeaders);
      dispatch({ type: CREATE_TASKLIST, payload: response.data });
    },
    getTask: async (taskListId: string, taskId: string) => {
      const response = await axios.get<Task>(`${BASE_URL}/api/task-lists/${taskListId}/tasks/${taskId}`, jsonHeaders);
      dispatch({ type: GET_TASK, payload: { taskListId, task: response.data } });
    },
    updateTaskList: async (id, taskList) => {
      const response = await axios.put<TaskList>(`${BASE_URL}/api/task-lists/${id}`, taskList, jsonHeaders);
      dispatch({ type: UPDATE_TASKLIST, payload: response.data });
    },
    deleteTaskList: async (id) => {
      await axios.delete(`${BASE_URL}/api/task-lists/${id}`, jsonHeaders);
      dispatch({ type: DELETE_TASKLIST, payload: id });
    },
    fetchTasks: async (taskListId) => {
      const response = await axios.get<Task[]>(`${BASE_URL}/api/task-lists/${taskListId}/tasks`, jsonHeaders);
      dispatch({ type: FETCH_TASKS, payload: { taskListId, tasks: response.data } });
    },
    createTask: async (taskListId, task) => {
      const response = await axios.post<Task>(`${BASE_URL}/api/task-lists/${taskListId}/tasks`, task, jsonHeaders);
      dispatch({ type: CREATE_TASK, payload: { taskListId, task: response.data } });
    },
